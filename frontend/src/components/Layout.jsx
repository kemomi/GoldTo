...
import { FileText, MessageCircle, Home, Cpu, Zap, Radio, Settings, Newspaper, BarChart3, Bell } from 'lucide-react'
      ...
      <aside className="w-56 flex-shrink-0 flex flex-col" style={{ background: 'var(--dark-800)', borderRight: '1px solid var(--border)' }}>
        ...
        <div className="p-5 flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-xs text-slate-500">情报分析引擎</div>
          </div>
        </div>

        <nav className="flex-1 px-3 pb-4 flex flex-col gap-1">
          <div className="text-xs text-slate-600 uppercase tracking-wider px-3 py-2 mt-2">情报分析</div>
          <NavItem to="/" icon={Home} label="首页" />
          <NavItem to={sessionId ? `/simulate/${sessionId}` : '#'} icon={Cpu} label="分析控制台" disabled={!sessionId} />
          <NavItem to={sessionId ? `/report/${sessionId}` : '#'} icon={FileText} label="情报简报" disabled={!sessionId} />
          <NavItem to={sessionId ? `/chat/${sessionId}` : '#'} icon={MessageCircle} label="深度对话" disabled={!sessionId} />

          <div className="text-xs text-slate-600 uppercase tracking-wider px-3 py-2 mt-4">每日战略简报</div>
          <NavItem to="/alerts" icon={Bell} label="情报预警" />
          <NavItem to="/dashboard" icon={BarChart3} label="数据看板" />
          <NavItem to="/briefings" icon={Newspaper} label="简报历史" />
        ...
        <div className="p-4 border-t" style={{ borderColor: 'var(--border)' }}>
          ...
          <div className="text-xs text-slate-600 mt-2">v2.0.0-daily</div>
        </div>
      </aside>
